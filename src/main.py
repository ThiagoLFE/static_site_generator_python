from src.copy_content import init
from src.utils.load_pages import load_pages
from os import getcwd
from os.path import join
import sys 
def main():
    # Pegar pasta remota e atribuir ao caminho
    basepath = sys.argv[1] if len(sys.argv) > 1 else "/"
    public_directory = "docs"
    
    root_path = getcwd()
    content_path = join(root_path, "content")
    path_template = join(root_path, "template.html")
    dst_path = join(root_path, public_directory)

    init(public_directory)
    load_pages(content_path, path_template, dst_path, basepath)

main()
